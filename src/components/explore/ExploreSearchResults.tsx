...
function FollowButton() {
  ...
  return (
    <button
      ...
      className={cn(
        'px-3 py-1.5 rounded-full text-[10px] font-medium uppercase tracking-[0.08em] transition-colors flex-shrink-0',
        isFollowing
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          <span className="text-[14px] text-white truncate">{a.displayName}</span>
...
function TagRow({ t }: { t: SearchTag }) {
  return (
    <button
      ...
      className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-white/[0.04] transition-colors text-left"
    >
      <span className="flex items-center justify-center w-10 h-10 rounded-full bg-white/[0.06] text-white/70 text-base font-semibold flex-shrink-0">
        #
      </span>
      <div className="flex-1 min-w-0">
        <p className="text-[14px] text-white truncate">#{t.name}</p>
        <p className="text-[11px] text-white/45">{formatCount(t.postCount)} posts</p>
      </div>
      <FollowTagButton />
    </button>
  );
    ...
    >
      <div className="flex items-center gap-2 mb-2">
        ...
        <div className="flex items-center gap-1.5 min-w-0 flex-1">
          <span className="text-[13px] text-white font-medium truncate">{p.author.displayName}</span>
      ...
      <p className="text-[14px] text-white/85 leading-relaxed mb-2 line-clamp-3">{p.body}</p>
      <div className="flex items-center gap-4 text-white/45 text-[11px] tabular-nums">
...
function EmptyState({ subtab, query }: { subtab: string; query: string }) {
  return (
    <div className="px-4 py-12 text-center">
      ...
      <p className="text-[12px] text-white/45 mt-1">Try a different search term.</p>
    </div>
  );
}
...
export function ExploreSearchResults({ query, submissionId }: ExploreSearchResultsProps) {
  ...
  const seeAllClass = 'text-[11px] font-medium uppercase tracking-[0.08em] hover:opacity-80 transition-opacity';
