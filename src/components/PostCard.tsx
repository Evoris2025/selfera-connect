          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-1 flex-wrap">
                <span className="font-bold text-foreground text-label drop-shadow-md">{author.name}</span>
                {author.isVerified && (
                  <EraVerifiedTooltip tier={calculateVerificationTier(0, false, author.email)} userEmail={author.email} size="xs" />
                )}
                {author.accountType && <AccountTypeBadge type={author.accountType} size="sm" />}
                <span className="text-foreground/70 text-caption font-normal drop-shadow-sm">@{author.handle}</span>
              </div>
            </motion.button>

            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon" className="h-7 w-7 rounded-full glass-subtle">
